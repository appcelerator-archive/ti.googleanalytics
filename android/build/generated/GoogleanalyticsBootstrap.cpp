...
#define TAG "ti.googleanalytics"

using namespace v8;

static Persistent<Object> bindingCache;

static void Googleanalytics_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Googleanalytics.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::GoogleanalyticsBindings::lookupGeneratedInit(
...
static void Googleanalytics_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Googleanalytics_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Googleanalytics_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::GoogleanalyticsBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry GoogleanalyticsBinding = {
	"ti.googleanalytics",
	Googleanalytics_init,
	Googleanalytics_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_googleanalytics_GoogleanalyticsBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.googleanalytics", &GoogleanalyticsBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::GoogleanalyticsBindings::lookupGeneratedInit));
}
